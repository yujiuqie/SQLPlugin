...
DOWNLOAD_URI=https://github.com/viktyz/SQLPlugin/releases/download/v1.1.1/SQLPlugin.tar.gz
...
BUNDLE_ID="com.alfredjiang.SQLPlugin"
TMP_FILE="$(mktemp -t ${BUNDLE_ID})"

# Remove SQLPlugin from Xcode's skipped plugins list if needed
if defaults read com.apple.dt.Xcode "$PLIST_PLUGINS_KEY" &> "$TMP_FILE"; then
    # We read the prefs successfully, delete SQLPlugin from the skipped list if needed
    /usr/libexec/PlistBuddy -c "delete skipped:$BUNDLE_ID" "$TMP_FILE" > /dev/null 2>&1 && {
        pgrep Xcode > /dev/null && {
            echo 'An instance of Xcode is currently running.' \
                 'Please close Xcode before installing SQLPlugin.'
            exit 1
        }
        defaults write com.apple.dt.Xcode "$PLIST_PLUGINS_KEY" "$(cat "$TMP_FILE")"
        echo 'SQLPlugin was removed from Xcode'\''s skipped plugins list.' \
...
# Download and install SQLPlugin
mkdir -p "${PLUGINS_DIR}"
...
echo "SQLPlugin successfully installed$omg_one_meme$beer " \
